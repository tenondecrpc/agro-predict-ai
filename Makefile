...
# ---------------------------------------------------------------------------
help:
	@echo "AgroPredict AI - Local development targets"
	@echo ""
	@echo "  Minikube:"
	...
	@echo "    make dev-worker           Run ARQ worker locally with uv"
	@echo ""
...
minikube-secrets:
	...
	kubectl create secret generic agropredict-ai-backend-secret \
...
smoke-test:
	...
	@curl -sf -X POST http://127.0.0.1:18000/api/v1/predictions \
		-H "Content-Type: application/json" \
		-d '{"tenant_id":"tenant-alpha","team_id":"team-core","crop":"corn","region":"midwest_us","time_horizon_days":30,"input_data":{"soil_moisture":0.35,"temperature_c":22.5,"rainfall_mm":45.0}}' \
		| python3 -m json.tool || echo "WARN: prediction endpoint returned non-200 (check backend readiness and model setup)"
	@echo ""
...
# ---------------------------------------------------------------------------
dev-worker:
	@echo "Starting ARQ worker locally..."
	@bash -c ' \
		if [ -z "$$BACKEND_ENCRYPTION_ACTIVE_KEY_ID" ]; then \
			export BACKEND_ENCRYPTION_ACTIVE_KEY_ID="kek-dev-v1"; \
			export BACKEND_ENCRYPTION_ACTIVE_WRAPPING_KEY=$$(python3 -c "from cryptography.fernet import Fernet; print(Fernet.generate_key().decode())"); \
...
local-worker-up:
	@echo "Starting ARQ worker via Docker Compose..."
	docker compose --profile worker up -d worker
	@echo "Worker started. Check logs: make local-worker-logs"
