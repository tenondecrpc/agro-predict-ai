...
export default function FlowSimulator({ candidate, reducedMotion }: Props) {
  ...
  function formatLogEntry(index: number) {
    ...
    const releaseLabel = step.emitsRecommendation ? "recommendation release step" : "no release";
    return `Step ${index + 1}: ${step.nodeId} (${step.label}) - transition ${step.transitionReason} - ${protectedLabel} - ${releaseLabel} - ${step.narration}`;
  }
        ...
        <>
          ...
          <section className="agent-step-summary" aria-label="Current step details">
            <dl className="agent-step-facts">
              ...
              <div>
                <dt>Recommendation release</dt>
                <dd>{currentStep.emitsRecommendation ? "Publishes recommendation" : "No external release"}</dd>
              </div>
            </dl>
        ...
        <div className="flow-simulator-node-strip" role="list" aria-label="Pipeline nodes">
          {plan.steps.map((step, i) => (
            ...
            >
              ...
              {step.emitsRecommendation ? (
                <span className="sim-node-release" aria-label="publishes recommendation">
                  R
                </span>
              ) : null}
            </div>
