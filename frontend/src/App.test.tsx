...
describe("App", () => {
  ...
  it("prediction flow simulator tab is hidden from viewer and visible to admin", () => {
    render(<App />);

    expect(screen.queryByRole("button", { name: "Prediction Flow Simulator" })).not.toBeInTheDocument();

    fireEvent.change(screen.getByLabelText("Role"), { target: { value: "admin" } });

    expect(screen.getByRole("button", { name: "Prediction Flow Simulator" })).toBeInTheDocument();
  });
...
describe("FlowSimulator", () => {
  ...
  it("renders dynamic office feedback with all agents visible", () => {
    ...
    expect(screen.getByLabelText("Agriculture logistics operations room: Prediction intake")).toBeInTheDocument();
    expect(screen.getByRole("listitem", { name: "Data Analyst active" })).toBeInTheDocument();
    expect(screen.getByRole("listitem", { name: "Reviewer idle" })).toBeInTheDocument();
    expect(screen.getByRole("listitem", { name: "ML Executor idle" })).toBeInTheDocument();
    expect(screen.getByRole("listitem", { name: "Recommendation idle" })).toBeInTheDocument();
    expect(screen.getByRole("listitem", { name: "Explainability idle" })).toBeInTheDocument();
    expect(screen.getByText("Protected workflow node")).toBeInTheDocument();
    expect(screen.getByText("No external release")).toBeInTheDocument();
    expect(screen.getByRole("log").textContent).toContain("ingest_event");
    expect(screen.getByRole("log").textContent).toContain("transition success");
  });

  it("shows collaborator interaction in prediction-oriented simulator steps", () => {
    render(<FlowSimulator candidate={activeGraphCandidate} reducedMotion={false} />);
    ...
    expect(screen.getByLabelText("Agriculture logistics operations room: Feature analysis")).toBeInTheDocument();
    expect(screen.getByRole("listitem", { name: "Data Analyst active" })).toBeInTheDocument();
    expect(screen.getByRole("listitem", { name: "Reviewer active" })).toBeInTheDocument();
    expect(screen.getByText(/profiles agronomic and logistics inputs/)).toBeInTheDocument();
  });
  ...
  it("auto-advances every 5s in slow mode with fake timers", () => {
    ...
    expect(screen.getByRole("log").textContent).toContain("Ingest Event");

    act(() => {
      vi.advanceTimersByTime(4999);
    });
    let items = screen.getByRole("log").querySelectorAll("li");
    expect(items[0].textContent).toContain("Ingest Event");

    act(() => {
      vi.advanceTimersByTime(1);
    });
    items = screen.getByRole("log").querySelectorAll("li");
    expect(items[0].textContent).toContain("Data Quality Gate");
