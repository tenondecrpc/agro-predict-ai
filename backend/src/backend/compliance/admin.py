    ...
    *,
    ...
    def execute_retention_run(
        ...
        try:
            ...
            elif surface == "agent_executions":
                rows_affected = _run_agent_executions_retention(engine, cutoff, mode)
            else:
                rows_affected = 0
            ...
            telemetry.increment(
                "agropredict_retention_runs_total",
                surface=surface,
                tenant_id=tenant_id,
                mode=mode,
            )
            telemetry.set_gauge(
                "agropredict_retention_rows_deleted",
                float(rows_affected),
                surface=surface,
                tenant_id=tenant_id,
            )
            ...
            telemetry.increment(
                "agropredict_retention_run_failures_total",
                surface=surface,
                tenant_id=tenant_id,
            )
    ...
    def execute_tenant_delete(
        ...
        telemetry.increment(
            "agropredict_tenant_deletions_total",
            tenant_id=tenant_id,
        )
...
def _run_agent_executions_retention(engine: Engine, cutoff: datetime, mode: str) -> int:
    """Delete or count agent_executions rows older than the cutoff."""
    ...
    with engine.begin() as connection:
        if mode == "enforce":
            result = connection.execute(
                text(
                    "DELETE FROM agent_executions WHERE started_at < :cutoff"
                ),
                {"cutoff": cutoff},
            )
            return result.rowcount
            ...
            result = connection.execute(
                select(func.count()).select_from(text("agent_executions")).where(
                    text("started_at < :cutoff")
...
def purge_agent_executions(
    database_url: str,
    retention_days: int = 90,
) -> int:
    """Delete agent_executions rows older than retention_days."""
    import os

    from sqlalchemy import text

    retention_days = int(os.getenv("AGENT_EXECUTION_RETENTION_DAYS", str(retention_days)))

    engine = create_engine(database_url, future=True, pool_pre_ping=True)
    with engine.begin() as conn:
        result = conn.execute(
            text(
                "DELETE FROM agent_executions WHERE started_at < NOW() - INTERVAL ':days days'"
            ),
            {"days": retention_days},
        )
