...
class KnowledgeIngestionService:
    ...
    def _record_progress(self, job: KnowledgeIngestionJob, *, count_terminal: bool = False) -> None:
        ...
        self.telemetry.set_gauge(
            "agropredict_knowledge_ingestion_progress_ratio",
        ...
        self.telemetry.set_gauge(
            "agropredict_knowledge_ingestion_processed_chunks",
        ...
        if count_terminal and job.status == "completed":
            self.telemetry.increment(
                "agropredict_knowledge_ingestion_jobs_total",
                tenant_id=job.tenant_id,
                team_id=job.team_id,
                status=job.status,
            )
...
class KnowledgeRetrievalService:
    ...
    def _record_retrieval_metrics(
        ...
        self.telemetry.observe("agropredict_knowledge_retrieval_latency_seconds", latency_seconds, **labels)
        self.telemetry.increment("agropredict_knowledge_retrieval_requests_total", **labels)
        if response.hits:
            self.telemetry.increment("agropredict_knowledge_retrieval_hits_total", **labels)
        self.telemetry.set_gauge(
            "agropredict_knowledge_retrieval_hit_ratio",
            1.0 if response.hits else 0.0,
            **labels,
        )
        for hit in response.hits:
            self.telemetry.observe(
                "agropredict_knowledge_excerpt_size_chars",
