...
class PostgresRedisWebhookGuard:
    ...
    def verify(self, request: WebhookRequest, *, now: int) -> WebhookGuardResult:
        with self._telemetry.trace(
            ...
            if matched_previous:
                self._telemetry.increment("agropredict_webhook_signature_matched_previous_total")
            ...
            if self._check_ticket_flood(request, now=now):
                ...
                self._telemetry.increment("agropredict_webhook_rate_limit_rejections_total")
            ...
            if self._cache_seen(idempotency_key):
                self._telemetry.increment("agropredict_webhook_dedupe_hits_total")
