...
def build_in_memory_persistence() -> PersistenceAdapters:
    ...
    return PersistenceAdapters(
        ...
        health=health,
        telemetry=telemetry,
        prediction_repository=build_prediction_repository(database, redis),
    )
...
def build_prediction_repository(
    database: DatabaseRuntime,
    redis: RedisRuntime,
) -> Any:
    """Build a prediction repository.

    Uses PostgreSQL-backed repository when infrastructure is configured,
    otherwise falls back to in-memory for local development.
    """
    if database.configured and redis.configured:
        from backend.predictions.repository import PostgresPredictionRepository
        return PostgresPredictionRepository(database_url=database.settings.sync_url())
