...
class PersistenceTelemetry:
    ...
    def trace(self, name: str, **attributes: str | None) -> Iterator[None]:
        ...
        try:
            ...
            self.observe(
                "agropredict_persistence_operation_seconds",
                duration,
                subsystem=subsystem,
                operation=operation,
            )
...
def bootstrap_telemetry(
    *,
    ...
    telemetry.set_gauge("agropredict_database_pool_utilisation_ratio", pool_utilisation)
    telemetry.set_gauge("agropredict_database_pool_wait_p95_seconds", pool_wait_p95)
    telemetry.set_gauge("agropredict_redis_command_p95_seconds", 0.0)
    telemetry.set_gauge("agropredict_provider_circuit_breaker_state", 0.0, provider_id="bootstrap")
    telemetry.set_gauge("agropredict_dlq_depth", 0.0)
    telemetry.increment("agropredict_webhook_dedupe_hits_total", 0.0)
    telemetry.increment("agropredict_budget_reservation_denials_total", 0.0)
    telemetry.set_gauge("agropredict_encryption_rotation_sla_ratio", 1.0)
    telemetry.set_gauge("agropredict_encryption_rotation_due_total", 0.0)
    telemetry.set_gauge(
