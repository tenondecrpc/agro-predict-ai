        ...
        with self._telemetry.trace(
            ...
            if success != 1:
                self._telemetry.increment(
                    "agropredict_budget_reservation_denials_total",
                    tenant_id=context.tenant_id,
                    team_id=context.team_id,
                )
                with self._scoped_transaction(context.tenant_id, context.team_id) as connection:
    ...
    def run_reconciliation(
        ...
        *,
        ...
        self._telemetry.set_gauge(
            "agropredict_billing_drift_percentage",
            float(drift_percentage),
            tenant_id=tenant_id,
            provider=provider,
        )
        self._telemetry.increment(
            "agropredict_reconciliation_runs_total",
            tenant_id=tenant_id,
            provider=provider,
            mode=mode,
        )
        if abs(drift_percentage) > 2:
            self._telemetry.increment(
                "agropredict_billing_drift_alerts_total",
                tenant_id=tenant_id,
                provider=provider,
            )
        return report
    ...
    def snapshot(self, provider_id: str):
        with self._telemetry.trace(
            ...
            try:
                ...
                probes = 0
            self._telemetry.set_gauge(
                "agropredict_provider_circuit_breaker_state",
