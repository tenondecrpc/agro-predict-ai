...
def test_knowledge_ingestion_jobs_are_resumable_and_emit_progress_metrics() -> None:
    ...
    assert "agropredict_knowledge_ingestion_progress_ratio" in metrics
    assert "agropredict_knowledge_ingestion_jobs_total" in metrics
...
def test_runtime_retrieval_is_tenant_scoped_and_role_whitelisted() -> None:
    ...
    metrics = telemetry.render_prometheus()
    assert "agropredict_knowledge_retrieval_latency_seconds" in metrics
    assert "agropredict_knowledge_retrieval_hit_ratio" in metrics
    assert "agropredict_knowledge_excerpt_size_chars" in metrics
