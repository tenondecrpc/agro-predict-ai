...
def test_deprecated_route_emits_headers_and_deprecation_metric() -> None:
    ...
    metric_line = (
        'agropredict_api_deprecation_hits_total{route="/api/v1/runs/{run_id}",'
        'tenant_id="tenant-alpha",version="v1"} 1.0'
    )
    assert metric_line in metrics.text
    assert 'agropredict_api_deprecation_sunset_days{route="/api/v1/runs/{run_id}",version="v1"}' in metrics.text
